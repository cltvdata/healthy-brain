...
const INITIAL_SOURCES: KnowledgeSource[] = [
  {
    id: 'src-pomelli-labs-1',
    title: 'Pomelli by Google Labs: Marketing AI & Estrategia Creativa',
    type: 'Web',
    priority: 1,
    priorityLabel: 'Google Labs (Prioridad 1)',
    sourceName: 'https://share.google/fREH49CehUNr4HIUk',
    status: 'Sincronizado',
    extract: 'Sincronización directa de activos creativos, posicionamiento de marca de bio-optimización y campañas generadas con la herramienta Pomelli by Google Labs.'
  },
  {
    id: 'src-gemini-share-3',
    title: 'Gemini Shared Artifact: Protocolos N3GS & Optimización de Longevidad',
    type: 'Web',
    priority: 1,
    priorityLabel: 'Grado Médico (Prioridad 1)',
    sourceName: 'https://share.gemini.google/N3GS5jVlIB9Z',
    status: 'Sincronizado',
    extract: 'Sincronización interactiva de prompts de Gemini para optimización cognitiva, seguimiento metabólico y bio-hacking asistido para la plataforma Healthy + Brain.'
  },
  {
    id: 'src-gemini-share-2',
    title: 'Gemini Shared Artifact: Arquitectura V67p & Protocolos Clínicos',
    type: 'Web',
    priority: 1,
    priorityLabel: 'Grado Médico (Prioridad 1)',
    sourceName: 'https://share.gemini.google/V67pGJnUBWiL',
    status: 'Sincronizado',
    extract: 'Protocolo avanzado de sincronización neuronal y metabólica, integración de bio-métricas en tiempo real, optimización de descanso profundo y hábitos automatizados para longevidad.'
  },
  {
    id: 'src-gemini-share-1',
    title: 'Gemini Shared Artifact: Modelo Integrado Healthy + Brain',
    type: 'Web',
    priority: 1,
    priorityLabel: 'Grado Médico (Prioridad 1)',
    sourceName: 'https://share.gemini.google/oz7m9skKdDaV',
    status: 'Sincronizado',
    extract: 'Análisis clínico de optimización de cortisol, arquitectura de neuro-tokens NTK y gemelo digital biológico con privacidad descentralizada y sincronización sin esfuerzo en Firebase.'
  },
  {
    id: 'src-notebooklm-artifact-1',
    title: 'NotebookLM Artifact: Resumen Clínico & Audio Overview',
    type: 'Web',
    priority: 1,
    priorityLabel: 'Grado Médico (Prioridad 1)',
    sourceName: 'https://notebooklm.google.com/notebook/30b16231-f630-4af3-af57-7efb9e8bde84/artifact/a58d9d90-a747-4e8e-ae8b-c0f61a0968f4',
    status: 'Sincronizado',
    extract: 'Artifact de NotebookLM sintetizado con diálogo médico de doble voz sobre ritmos circadianos, gestión metabólica de la glucosa y estimulación vagal adaptativa por hápticos.'
  },
  {
    id: 'src-1',
...
export const NotebookLMTab: React.FC<NotebookLMTabProps> = ({ onAddTokens, onUpdateMetrics, tokens, stitchMode }) => {
  ...
  const [podcastLog, setPodcastLog] = useState<string>('Resumen clínico listo para sintetizar.');
    ...
    <div className="space-y-8 animate-fade-in pb-20">
      ...
      <div className={`relative overflow-hidden rounded-3xl p-6 border transition-all duration-700 ${
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-2">
          <span className={`px-2 py-0.5 rounded-md text-[9px] font-black uppercase tracking-widest inline-block border w-fit ${
            stitchMode === 'night' 
          ...
          <a 
            ...
            className="px-2.5 py-1 bg-white/5 hover:bg-white/10 text-gray-300 border border-white/10 rounded-lg text-[10px] font-bold uppercase tracking-wider flex items-center gap-1.5 transition-all w-fit"
          >
            <ExternalLink className="w-3 h-3 text-bio-orange" /> Stitch Project #1336072952115328009 ↗
          </a>
        </div>
      ...
      {/* SINTETIZADOR DE PODCAST & AUDIO DEEP DIVE DE NOTEBOOKLM */}
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-2xl bg-indigo-500/20 text-indigo-400 flex items-center justify-center border border-indigo-500/30">
              <Volume2 className={`w-5 h-5 ${isPlayingPodcast ? 'animate-bounce text-bio-green' : 'text-indigo-400'}`} />
            </div>
            <div>
              <h3 className="text-sm font-black uppercase text-white">NotebookLM Audio Overview / Podcast Deep Dive 🎙️</h3>
              <p className="text-[10px] text-gray-400 font-bold uppercase tracking-wider mt-0.5">Sintetizador de Diálogos Clínicos & Resumen Ejecutivo</p>
            </div>
          </div>
          ...
          <button
            onClick={() => {
              if (isPlayingPodcast) {
                ...
                setPodcastLog('Audio pausado por el usuario.');
              } else {
                setIsPlayingPodcast(true);
                setPodcastProgress(10);
                setPodcastLog('Sintetizando episodio con voces clínicas duales (NotebookLM Audio Engine)...');
                onAddTokens(50, "Sintetización de Audio Overview NotebookLM");

                let prog = 10;
                const interval = setInterval(() => {
                  prog += 20;
                  setPodcastProgress(prog);
                  if (prog >= 100) {
                    clearInterval(interval);
                    setIsPlayingPodcast(false);
                    setPodcastLog('✓ Reproducción de Audio Overview completada (+50 NTK).');
                  }
                }, 1200);
              }
            }}
            className={`py-2.5 px-4 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2 ${
              isPlayingPodcast 
                ? 'bg-bio-green/20 border border-bio-green text-bio-green animate-pulse' 
                : 'bg-indigo-600 hover:bg-indigo-500 text-white shadow-lg shadow-indigo-600/20'
            }`}
          >
            <Radio className="w-4 h-4" />
            {isPlayingPodcast ? 'Reproduciendo Overview...' : 'Generar Audio Overview (2 min)'}
          </button>
        </div>

        <div className="p-4 bg-black/40 border border-white/5 rounded-2xl space-y-3">
          <div className="flex justify-between items-center text-[10px] font-bold text-gray-400">
            <span>{podcastLog}</span>
            <span>{podcastProgress}%</span>
          </div>
          <div className="w-full bg-white/5 h-2 rounded-full overflow-hidden">
            <div 
              className="h-full bg-gradient-to-r from-indigo-500 to-bio-green rounded-full transition-all duration-500" 
              style={{ width: `${podcastProgress}%` }}
            ></div>
          </div>
          <p className="text-xs text-gray-300 italic">
            "Anfitrión A: Bienvenida a este resumen de NotebookLM. Hoy analizamos la correlación entre la exposición a luz solar de 480nm y la estabilización de cortisol plasmático..."
          </p>
        </div>
      </div>
                  ...
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-1.5">
                        <span className="text-[9px] text-gray-400 font-bold uppercase truncate max-w-[220px] sm:max-w-[400px]">
                          {src.sourceName}
                        </span>
                        {src.sourceName.startsWith('http') && (
                          <a 
                            href={src.sourceName} 
                            target="_blank" 
                            rel="noopener noreferrer" 
                            className="text-primary hover:underline flex items-center gap-0.5 text-[9px] font-black uppercase"
                          >
                            <ExternalLink className="w-3 h-3" /> Abrir Link
                          </a>
                        )}
                        <span className="text-[9px] text-gray-500 font-bold uppercase">• {src.priorityLabel}</span>
                      </div>
                    </div>
                  </div>
                  <span className="text-[9px] font-black px-2 py-0.5 rounded-md bg-bio-green/15 text-bio-green border border-bio-green/30 uppercase tracking-wider flex-shrink-0">
                    {src.status}
                  </span>
                </div>
                <p className="text-[11px] text-gray-300 font-medium leading-relaxed bg-black/30 p-3 rounded-xl border border-white/5">
                  "{src.extract}"
                </p>
              </div>
            ))}
